...
4. **`-strong` 이 라이트에서 아무 값도 추가하지 않는다 — 항목 2 대비 실패의 원인.**
   소비자(axdiag admin)가 항목 2 의 대비 실패를 자기 별칭 레이어에서 고치려 했다. 소비자의
   별칭 가드는 색 리터럴을 금지하므로, 고치려면 정본 안에 더 진한 단계가 있어야 한다. 정본
   `dist/styles/index.css` 의 라이트 `:root` 블록을 직접 읽으면:
   ...
   --danger-strong:  var(--danger);    /* base 와 동일 */
   --success-strong: var(--success);   /* base 와 동일 */
   ```

   `--danger-strong` 과 `--success-strong` 은 **이름만 strong 이고 base 를 그대로
   가리킨다** — 라이트 모드에 진한 단계가 존재하지 않는다. 다크 블록에는 실제로 다른 값이
   있다(`--danger-strong: #f5475c`, `--success-strong: #4dd488`) — 즉 라이트만 비어 있다.
   `--warning-strong` 은 유일하게 다른 값(`#f4ab00`)인데, 이미 가장 진한 warning 단계인데도
   흰 배경에서 **1.97:1** 이라 그 자체로 AA 를 못 넘는다.

   **결과: 소비자가 상태색 대비를 고칠 수 없다.** 별칭으로 이을 진한 토큰이 정본에 없고,
   리터럴은 자기 가드가 막는다. 항목 2 의 실패들이 "정본 유래"인 이유가 이것이다 — 소비자
   잘못이 아니라 우회로가 없는 것이다. 실제로 소비자는 **고칠 수 있었던 것은 고쳤다**:
   axdiag 는 자기가 만든 조합 2건(`--fg-muted`|`--bg-tertiary` 4.48:1,
   `--fg-muted`|`--accent-soft` 4.26:1)을 별칭으로 고쳐 각각 7.11:1·6.75:1 로 올렸다.
   남은 것만 정본 사안이다.

   **이 공백은 항목 1(gray 200/300/400 부재)보다 발견하기 어렵다.** gray 는 정의가 없어서
   Tailwind 기본값이 새는 형태라 값을 재면 드러난다. 반면 `-strong` 은 **정의가 있어서**
   토큰 목록만 보면 "진한 단계가 준비돼 있다"고 읽히지만, 실제 값은 base 와 같아 아무것도
   추가하지 않는다. 이름이 약속한 것을 값이 지키지 않는 형태다.

   **필요한 것**: 라이트 `--danger-strong`·`--success-strong` 에 실제로 더 진한 값을 주고,
   `--warning-strong` 은 흰 배경에서 4.5:1 을 넘는 값으로 다시 잡아야 한다. 다크는 이미
   별도 값이 있으므로 라이트만 해당된다. (후속 작업 — 별도 PR. 이 PR 범위 밖이다.)

   **왜 소비자가 우회할 수 없나 — 정본에 상태색 계단이 아예 없다.** `dist/styles/index.css`
   에서 번호가 붙은 팔레트를 전수로 뽑으면 두 계열뿐이다:

   | 계열 | 단계 |
   |---|---|
   | `--color-accent-*` | 50 · 100 · 200 · 300 · 400 · 500 · 600 · 700 · 800 · 900 · 950 (11단계) |
   | `--color-gray-*` | 50 · 100 · 500 · 600 · 700 · 800 · 850 · 900 · 950 (9단계, 200~400 은 항목 1의 공백) |
   | `--color-danger-*` · `--color-success-*` · `--color-warning-*` | **없음 (0단계)** |

   accent 에는 11단계를 주면서 상태색에는 계단 자체를 안 준 비대칭이다. 소비자의 별칭
   레이어는 `var(--정본토큰)` 만 쓸 수 있고(자기 가드가 색 리터럴을 차단한다), 이을 진한
   상태색 토큰이 정본에 **존재하지 않으므로** 소비자 쪽에서 고칠 방법이 물리적으로 없다.
   accent 와 같은 방식으로 danger·success·warning 에 번호 팔레트를 주는 것이 가장
   곧은 해법이고, `-strong` 은 그 계단의 한 단계를 가리키게 하면 된다.
5. `--color-gray-*` 계단에 다크 분기가 없다. 회색을 배경/전경으로 직접 쓰는 소비자는
   다크에서 값이 안 바뀐다. 시맨틱 토큰(`--bg-*`/`--fg-*`)을 쓰라고 안내해야 한다.
6. `table` · `avatar` · `breadcrumbs` 어휘가 없다. axdiag `apps/admin` 이 자체 정의해 쓴다.
...
7. `List`·`SkeletonRow` 는 소비자가 0이다. 다음 major 에서 제거를 검토한다.
